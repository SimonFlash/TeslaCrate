...
public class Menu implements CommandExecutor {
    ...
    public static final CommandSpec COMMAND = CommandSpec.builder()
            .executor(new Menu())
            .arguments(GenericArguments.optional(Arguments.map("command", Storage.commands, Arguments.string("string"))))
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
        if (src instanceof Player) {
            if (args.hasAny("command")) {
                Utils.page(args.<Command>getOne("command").get().getMenuElements()).open((Player) src);
            } else {
                Utils.menu(Storage.commands.values()).open((Player) src);
            }
