...
public class Menu implements CommandExecutor {
    ...
    public static final CommandSpec COMMAND = CommandSpec.builder()
            .executor(new Menu())
            .arguments(GenericArguments.optional(Arguments.map("key", Storage.keys, Arguments.string("string"))))
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
        if (src instanceof Player) {
            if (args.hasAny("key")) {
                Utils.page(args.<Key>getOne("key").get().getMenuElements()).open((Player) src);
            } else {
                Utils.menu(Storage.keys.values()).open((Player) src);
            }
