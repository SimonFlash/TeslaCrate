...
public class SpiralPath extends AnimationPath {

    private final int precision;

    public SpiralPath(int segments, int precision) {
        super(segments);
    ...
    public Vector3f[] getPositions(float radians) {
        ...
        for (float shift : AnimUtils.shift(radians, AnimUtils.shift(getSegments()))) {
            for (radians = 0; radians < AnimUtils.TAU; radians += AnimUtils.shift(precision)) {
